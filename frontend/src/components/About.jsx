...
const HighlightChip = ({ text }) => (
  <div className="flex items-start gap-3 p-4 rounded-xl glass hover:border-cyan-400/30 transition-colors">
...
const AboutContent = ({ about }) => (
  <div className="lg:col-span-7 reveal space-y-5">
    {about.paragraphs.map((p) => (
      <p key={p.slice(0, 24)} className="text-slate-300 text-lg leading-relaxed">{p}</p>
    ))}
    <div className="pt-6 grid sm:grid-cols-2 gap-3">
      {about.highlights.map((h) => (
        <HighlightChip key={h} text={h} />
      ))}
    </div>
  </div>
);
...
const LeadershipCard = ({ involvement }) => (
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-14 grid lg:grid-cols-12 gap-10">
          ...
          <aside className="lg:col-span-5 reveal space-y-4">
            ...
            <LeadershipCard involvement={involvement} />
          </aside>
        </div>
      </div>
    </section>

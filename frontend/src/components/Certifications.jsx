...
const CertCard = ({ cert }) => (
  <div className="glass rounded-2xl p-6 hover:border-cyan-400/30 transition-colors group">
    <div className="flex items-start gap-4">
      ...
      <div className="flex-1">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          ...
          <span className="font-mono text-xs px-2 py-0.5 rounded-md bg-white/5 text-slate-300 border border-white/10">{cert.year}</span>
        </div>
        <div className="text-cyan-400 text-sm mt-0.5">{cert.issuer}</div>
        <p className="text-slate-300 text-sm mt-3 leading-relaxed">{cert.summary}</p>
      </div>
    </div>
  </div>
);
...
const AchievementCard = ({ item }) => (
  <div className="glass rounded-2xl p-6 hover:border-amber-300/30 transition-all hover:-translate-y-1">
    <div className="font-display text-4xl font-bold text-amber-300 mb-2">{item.value}</div>
    <div className="text-sm text-slate-300 leading-relaxed">{item.text}</div>
  </div>
);

const CertificationsColumn = ({ items }) => (
  ...
  <section id="certifications" className="relative py-24 px-6">
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mt-14 grid lg:grid-cols-2 gap-10">
        <CertificationsColumn items={portfolioData.certifications} />
        <ImpactColumn />
      </div>
    </div>
  </section>
